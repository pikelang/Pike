...
if [ "x1" == "x`git remote -v  | egrep -c "^origin\s+git://pike-git.lysator.liu.se/pike\s+\(push\)$"`" ]; then
  echo git remote add --mirror -f origin git://pike-git.lysator.liu.se/pike
fi

if [ "x1" == "x`git remote -v  | egrep -c "^github\s+git@github.com:pikelang/Pike.git\s+\(push\)$"`" ]; then
  echo git remote add -f github git@github.com:pikelang/Pike.git
fi

git fetch -q origin
git fetch -q github
git checkout github_sync
