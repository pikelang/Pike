 ...
 *
 * This table was initially generated by correlating the behavior of
 * _dosmaperr() with the <WinError.h> and <errno.h> header files.
 *
 * It has afterwards been extended with some special cases not present
 * in the original _dosmaperr() and with remappings of WSA-errors.
 *
 * Entries not in the table that are less than 1000 map to EINVAL,
 * other values (eg WSAE*) map to themselves.
 *
 * FIXME: The table ought to be extended to map the remaining
 *        error codes less than 1000.
 */
static const unsigned long pike_doserrtab[][2] = {
  ...
  /* WinSock errnos. These are mapped to their plain variants when possible.
   *
   * Old windows <crt/errno.h> defines only a few of these.
   *
   * Recent windows <ucrt/errno.h> on the other hand defines most of them.
   *
   * CygWin and Mingw's <errno.h> define all but a few of them.
   *
   * Note that recent windows defines an EWOULDBLOCK (140) that differs
   * from EAGAIN (11). Do *NOT* use EWOULDBLOCK below.
   */
...
#if 0
  {  WSAEPROCLIM,			EPROCLIM,		}, /* 10067 */
  {  WSAEUSERS,				EUSERS,			}, /* 10068 */
  {  WSAEDQUOT,				EDQUOT,			}, /* 10069 */
  {  WSAESTALE,				ESTALE,			}, /* 10070 */
  {  WSAEREMOTE,			EREMOTE,		}, /* 10071 */
  {  WSAEDISCON,			EDISCON,		}, /* 10101 */
  {  WSAENOMORE,			ENOMORE,		}, /* 10102 */
#endif
#ifdef ECANCELLED
  {  WSAECANCELLED,			ECANCELLED,		}, /* 10103 */
#endif
#if 0
  {  WSAEINVALIDPROCTABLE,		EINVALIDPROCTABLE,	}, /* 10104 */
  {  WSAEINVALIDPROVIDER,		EINVALIDPROVIDER,	}, /* 10105 */
  {  WSAEPROVIDERFAILEDINIT,		EPROVIDERFAILEDINIT,	}, /* 10106 */
  {  WSAEREFUSED,			EREFUSED,		}, /* 10112 */
#endif
};

/* Alternative implementation of _dosmaperr(), as it isn't provided
 * by all CRT runtimes; eg both MinGW and msvcr100.dll seem to not
 * implement _dosmaperr()...
 *
...
static inline int pike_dosmaperr(unsigned long err)
{
  ...
  /* Map error codes above 1000 verbatim.
   *
   * This lets the winsock codes that have not been mapped above
   * through as-is, e.g. WSAESHUTDOWN. There are symbolic
   * constants for all of them in the System module, but the
   * duplicate values still complicates code on the Windows platform.
   *
   * Note: In Pike 8.0 and earlier the cutoff was at STRUNCATE (80).
   *       It was raised to 1000 to make space and some margin for
   *       the POSIX Supplement errnos that were mapped in the range
...
PMOD_EXPORT void set_errno_from_win32_error (unsigned long err)
{
  ...
  /* pike_dosmaperr handles all common I/O errors from GetLastError,
