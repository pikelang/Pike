...
//
// Huffman code and number of significant bits.
constant huffman_tab = ({
  ({     0x1ff8, 13, }),
  ({   0x7fffd8, 23, }),
  ({  0xfffffe2, 28, }),
  ({  0xfffffe3, 28, }),
...
{
  // Validate the table.
  if (sizeof(huffman_tab) != 257) {
    error("Wrong number of entries: %d!\n",
	  sizeof(huffman_tab));
  }
...
/* Huffman tables.
 *
 * This file is automatically generated from mk_huffman.pike.
 *
 * Do NOT edit.
 */
struct huffentry {
...
static const struct huffentry pack_tab[] = {\n");
  ...
  // Sort the table for binary search during decoding.
  sort(column(pack_tab, 0), pack_tab);
  out->add(#"
