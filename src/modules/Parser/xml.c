...
|| $Id: xml.c,v 1.84 2006/05/02 18:19:10 grubba Exp $
*/
...
#define POP() do {							\
    ...
    really_free_xmlinput(data->input);					\
    data->input=i;							\
...
#define XMLERROR(desc) do {xmlerror(desc,data,NULL); READ (1);} while (0)
#define XMLERROR_STAY(desc) xmlerror(desc,data,NULL)
...
static void parse_xml(INT32 args)
{
  ...
  data.input = alloc_xmlinput();
  data.input->datap = MKPCHARP_STR(s);
  data.input->len = s->len;
  data.input->pos = 0;
  data.input->to_free = NULL;
  data.input->entity = 0;
  ...
  if (doc_seq_pos != DOC_AFTER_ROOT_ELEM &&
    ...
    xmlerror ("Root element missing.", &data, NULL);
...
static void compat_allow_errors(INT32 args)
{
  ...
  if (UNSAFE_IS_ZERO (sp - args))
    ...
    MAKE_CONST_STRING (str_7_2, "7.2");
    MAKE_CONST_STRING (str_7_6, "7.6");
