...
#define DECODE_PROTOTYPE(PREFIX, FORMAT, DECODE_PREFIX)			\
  IMAGE_PROTOTYPE(FORMAT);						\
  void PIKE_CONCAT4(PREFIX, FORMAT, DECODE_PREFIX, _decode)(INT32 args)
#define DECODE_HEADER_PROTOTYPE(PREFIX, FORMAT, DECODE_PREFIX)          \
  DECODE_PROTOTYPE(PREFIX, FORMAT, DECODE_PREFIX);                      \
  void PIKE_CONCAT4(PREFIX, FORMAT, DECODE_PREFIX, decode_header)(INT32 args)
#define IMAGE_DECODE_PROTOTYPE(FORMAT, DECODE_PREFIX)			\
  DECODE_PROTOTYPE(image_, FORMAT, DECODE_PREFIX)
#define IMAGE_DECODE_HEADER_PROTOTYPE(FORMAT, DECODE_PREFIX)		\
  DECODE_HEADER_PROTOTYPE(image_, FORMAT, DECODE_PREFIX)
#define IMG_DECODE_PROTOTYPE(FORMAT, DECODE_PREFIX)			\
  DECODE_PROTOTYPE(img_, FORMAT, DECODE_PREFIX)
#define IMG_DECODE_HEADER_PROTOTYPE(FORMAT, DECODE_PREFIX)		\
  DECODE_HEADER_PROTOTYPE(img_, FORMAT, DECODE_PREFIX)

#ifndef EMPTY_SYMBOL
/* Needed to make MSVC's cpp happy. */
#define EMPTY_SYMBOL
#endif
