  ...
  ADD_EFUN2("`+",f_add,
	    tOr7(tIfnot(tFuncV(tNone,tNot(tObj),tMix),
		 ...
		 tIfnot(tFuncV(tNone, tNot(tStr), tMix),
			tFuncV(tOr3(tNStr(tSetvar(0, tInt)),tInt,tFlt),
...
#define TRANS_NAME	"`+"
  ...
  ADD_EFUN2("`-",f_minus,
	    tOr8(tIfnot(tFuncV(tNone,tNot(tObj),tMix),
			tOr(tFuncArg(tSetvar(2,tObj), tFindLFun(tVar(2), "`-")),
			    tFuncV(tNot(tObj), tMix, tMix))),
		 tOr(tFuncArg(tIntNeg, tOr(tFuncV(tNone, tIntPos, tIntNeg),
					   tIfnot(tFuncV(tNone, tNot(tIntNeg),
							 tMix),
						  tFuncV(tInt, tInt, tInt)))),
		     tFuncArg(tIntPos, tOr(tFuncV(tNone, tIntNeg, tIntPos),
					   tIfnot(tFuncV(tNone, tNot(tIntPos),
							 tMix),
						  tFuncV(tInt, tInt, tInt))))),
		 tIfnot(tFuncV(tNone,tNot(tFlt),tMix),
			tFuncV(tOr(tInt,tFlt),tOr(tInt,tFlt),tFlt)),
...
#define SHIFT_TYPE							\
  ...
  */
  ...
  ADD_EFUN2("`*", f_multiply,
	    tOr8(tIfnot(tFuncV(tNone,tNot(tObj),tMix),
			tFuncV(tMix,tOr(tMix,tVoid),tMix)),
		 tFunc(tArr(tArr(tSetvar(1,tMix)))
		       tArr(tSetvar(1,tMix)),tArr(tVar(1))),
		 tOr6(tIfnot(tFuncV(tNone,tNot(tZero),tMix),
			     tFuncV(tNone, tInt, tZero)),
		      tFuncV(tInt1Plus,tInt1Plus,tInt1Plus),
		      tFuncV(tIntMinus,tInt1Plus,tIntMinus),
