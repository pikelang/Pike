...
static INLINE int mapping_can_destruct(const struct mapping * m) {
    return m->refs == 1 && !(m->flags & MAPPING_WEAK);
}
...
static INLINE void mark_check(struct reentrance_marker * m) {
    if (m->marker) Pike_error("Object callback is not side effect free: %u!\n", m->marker);
}
...
static void mapping_grow(struct mapping * m) {
    ...
    unsigned INT32 magnitude = m->magnitude;
    unsigned INT32 i;
    unsigned INT32 size = m->size / AVG_CHAIN_LENGTH;

#ifdef PIKE_DEBUG
    check_mapping(m);
#endif

    while (size > hash_mask) {
	magnitude --;
	hash_mask *= 2;
    }

    m->table = (struct keypair**)xrealloc(m->table, sizeof(struct keypair *) * hash_mask);
    MEMSET((char*)(m->table + m->hash_mask + 1), 0,
	   sizeof(struct keypair *) * (hash_mask - 1 - m->hash_mask));

    i = m->hash_mask;

    m->hash_mask = hash_mask - 1;
    m->magnitude = magnitude;

    for (;; i--) {
	struct keypair ** t = m->table + i;
	struct keypair ** slot = t;

	while (*t) {
	    struct keypair ** slot2 = low_get_bucket(m, (*t)->hval);
...
static void mapping_shrink(struct mapping * m) {
}

static INLINE void mapping_try_rehash(struct mapping * m) {
    unsigned INT32 hsize = AVG_CHAIN_LENGTH * (m->hash_mask + 1);

    if (m->size >= 2 * hsize) {
	mapping_grow(m);
    } else if (m->size < hsize / 2) {
	mapping_shrink(m);
    }
}
...
void mapping_rel_simple(void * _start, void * _stop, ptrdiff_t diff, void * data) {
    ...
    /*
     * NOTE: this function is only called once, unless we start using ba_lreserve
     * reserving more than one element.
     */
    ba_simple_rel_pointer((char*)&m->trash, diff);

    for (it = m->first_iterator; it; it = it->next) {
	/*
	 * u.current might point to the hash table itself
	 */
	if ((void*)it->u.current < _stop && (void*)it->u.current >= _start)
	    ba_simple_rel_pointer((char*)&it->u.current, diff);
    }
...
#endif
    free(m->table);
    if ((m->val_types | m->key_types) & BIT_REF_TYPES)
	ba_walk_local(&m->allocator, free_keypairs, NULL);
    ba_ldestroy(&m->allocator);
    DOUBLE_UNLINK(first_mapping, m);
 ...
 * code that reenters should not call this. TODO: this function also takes care
...
#ifdef PIKE_DEBUG
	if (keypair_deleted(k)) {
	    Pike_error("ran into deleted keypair.");
	}
#endif
...
static struct keypair ** really_low_mapping_lookup(struct mapping *m,
						   const struct svalue * key) {
    const unsigned INT32 hval = low_mapping_hash(key);
    struct keypair ** t;
    int frozen = 0;
...
static INLINE void mapping_it_set(struct mapping_iterator * it, const struct svalue * key) {
    struct keypair ** t = really_low_mapping_lookup(it->m, key);
    it->u.k = t ? *t : NULL;
}
...
PMOD_EXPORT void map_delete_no_free(struct mapping *m, const struct svalue *key,
				    struct svalue *to) {
    struct keypair ** t = really_low_mapping_lookup(m, key);
    ...
    if (to) {
	move_svalue(to, &(*t)->u.val);
    } else {
	free_svalue(&(*t)->u.val);
    }

    mapping_delete_slot(m, t);
    mapping_try_rehash(m);
}
...
/* TODO
 * The above functions iterate over the mapping completely. they should update the type
 * fields
 */
...
static void destructive_or_mappings(struct mapping *a, const struct mapping *b) {
    unsigned INT32 e;
    struct keypair * k;

    MAPPING_LOOP(b) {
	low_mapping_insert(a, &k->key, &k->u.val, 2);
    }
}
...
static struct mapping *or_mappings(struct mapping *a, struct mapping *b) {
    ...
    if (!b->size) return destructive_copy_mapping(a);
    ...
    ka = mapping_it_current(&c.ita);
    kb = mapping_it_current(&c.itb);
	 ...
	 */
	for (s = kb; s && s->hval == ka->hval; s = s->u.next) {
	    if (is_identical(&s->key, &ka->key)) goto skip;
	    if (is_nidentical(&s->key, &ka->key)) continue;
	    if (is_eq(&s->key, &ka->key)) goto skip;
	}
insert:
...
static struct mapping *and_mappings(struct mapping *a, struct mapping *b) {
	 ...
	 */
	for (s = kb; s && s->hval == ka->hval; s = s->u.next) {
	    if (is_identical(&s->key, &ka->key)) goto insert;
	    if (is_nidentical(&s->key, &ka->key)) continue;
	    if (is_eq(&s->key, &ka->key)) goto insert;
	}
insert:
	mapping_builder_add(&c.builder, *t);
skip:
...
static void destructive_subtract_mappings(struct mapping *a, const struct mapping *b) {
    unsigned INT32 e;
    struct keypair *k;

    MAPPING_LOOP(b) {
	map_delete_no_free(a, &k->key, NULL);
    }
}
...
static struct mapping *subtract_mappings(struct mapping *a, struct mapping *b) {
    ...
    if (!b->size || !a->size) return destructive_copy_mapping(a);
	...
	/*
	 * scan through all keypair with identical hash value and
	 * skip those which are also in the other mapping. they
	 * will be skipped when iterated over by the code above
	 */
	for (s = kb; s && s->hval == ka->hval; s = s->u.next) {
	    if (is_identical(&s->key, &ka->key)) goto skip;
...
static INLINE void xor_bucket(struct keypair * k1, struct keypair * k2, struct mapping_iterator * b) {
    ...
    for (; k1 && k1->hval == k2->hval; k1 = k1->next) {
	for (s = k2; s && s->hval == k1->hval; s = s->next) {
...
static struct mapping *xor_mappings(struct mapping *a, struct mapping *b) {
    ...
    /* TODO: maybe this is too much, but relocation is rather expensive */
    ret = debug_allocate_mapping(b->size + a->size);
...
PMOD_EXPORT struct mapping *merge_mappings(struct mapping *a, struct mapping *b, INT32 op) {
  ...
  case PIKE_ARRAY_OP_SUB:
    if (mapping_can_destruct(a)) {
	add_ref(a);
	destructive_subtract_mappings(a, b);
	return a;
    }
...
PMOD_EXPORT struct mapping *merge_mapping_array_ordered(struct mapping *a, struct array *b, INT32 op) {
	...
	a = destructive_copy_mapping(a);

	for (i = 0; i < b->size; i++) {
	    map_delete_no_free(a, key++, NULL);
	}

	mapping_try_rehash(a);
...
PMOD_EXPORT struct mapping *merge_mapping_array_unordered(struct mapping *a, struct array *b, INT32 op) {
    return merge_mapping_array_ordered(a, b, op);
}
...
PMOD_EXPORT struct mapping *add_mappings(struct svalue *argp, INT32 args)
{
  ...
  for(;d<args;d++)
  {
    ...
    if(!(m->flags  & MAPPING_WEAK))
    {
      ...
      if(e==m->size)
	return destructive_copy_mapping(m);
#endif
...
PMOD_EXPORT struct mapping *copy_mapping_recursively(struct mapping *m,
  ...
  if((m->val_types | m->key_types) & BIT_COMPLEX) {
    ...
    not_complex = 1;
    ret = destructive_copy_mapping(m);
  }
...
void check_mapping(const struct mapping *m) {
    unsigned INT32 count = m->size;
    unsigned INT32 used = (unsigned INT32)ba_lcount(&m->allocator);
    struct keypair * k;
    unsigned INT32 i;

    for (k = m->trash; k; k = k->u.next) count++;

    if (used != count) {
	fprintf(stderr, "allocator has %u blocks, but size is %u\n", used, count);
    }

    if (m->hash_mask != ~(unsigned INT32)0 >> m->magnitude) {
	fprintf(stderr, "hash_mask %x and magnitude %u dont match.\n", m->hash_mask,
		m->magnitude);
    }

    count = m->size;
    for (i = 0; i <= m->hash_mask; i++) {
	struct keypair * k = m->table[i];
	unsigned INT32 hval = 0;

	while (k) {
	    count--;
	    if (hval > k->hval) {
		fprintf(stderr, "%p is not sorted properly: %u > %u\n", k, hval, k->hval);
	    }
	    if (m->table + i != low_get_bucket(m, k->hval)) {
		fprintf(stderr, "%p is in wrong bucket %p[%u] vs %p\n", k, m->table + i,
			i, low_get_bucket(m, k->hval));
	    }
	    k = k->next;
	}
    }
    ...
    if (count) {
	fprintf(stderr, "entries got lost. %u vs %u\n", m->size, m->size - count);
    }
}
...
#define GC_RECURSE(M, REC_KEYPAIR, TYPE, IND_TYPES, VAL_TYPES) do {	\
  int remove;								\
  unsigned INT32 i;							\
  for (i = 0; i <= (M)->hash_mask; i++) {				\
      struct keypair ** t = m->table + i;				\
...
void gc_mark_mapping_as_referenced(struct mapping *m) {
  ...
  if(gc_mark(m, T_MAPPING))
	...
	case 0:
	  GC_RECURSE(m, GC_REC_KP, gc_mark, key_types, val_types);
	  gc_assert_checked_as_nonweak(m);
	  break;
	case MAPPING_WEAK_INDICES:
	  GC_RECURSE(m, GC_REC_KP_IND, gc_mark, key_types, val_types);
	  gc_assert_checked_as_weak(m);
	  break;
	case MAPPING_WEAK_VALUES:
	  GC_RECURSE(m, GC_REC_KP_VAL, gc_mark, key_types, val_types);
	  gc_assert_checked_as_weak(m);
	  break;
	default:
	  GC_RECURSE(m, GC_REC_KP_BOTH, gc_mark, key_types, val_types);
	  gc_assert_checked_as_weak(m);
	  break;
	}
	m->val_types = val_types;
...
void real_gc_cycle_check_mapping(struct mapping *m, int weak) {
  GC_CYCLE_ENTER(m, T_MAPPING, weak) {
	...
	switch (m->flags & MAPPING_WEAK) {
	  case 0:
	    GC_RECURSE(m, GC_REC_KP, gc_cycle_check, key_types, val_types);
	    gc_assert_checked_as_nonweak(m);
	    break;
	  case MAPPING_WEAK_INDICES:
	    GC_RECURSE(m, GC_REC_KP_IND, gc_cycle_check, key_types, val_types);
	    gc_assert_checked_as_weak(m);
	    break;
	  case MAPPING_WEAK_VALUES:
	    GC_RECURSE(m, GC_REC_KP_VAL, gc_cycle_check, key_types, val_types);
	    gc_assert_checked_as_weak(m);
	    break;
	  default:
	    GC_RECURSE(m, GC_REC_KP_BOTH, gc_cycle_check, key_types, val_types);
	    gc_assert_checked_as_weak(m);
	    break;
	}
...
struct keypair_foreach_cb {
    void (*kfun)(const struct svalue *, size_t);
    void (*vfun)(const struct svalue *, size_t);
};
...
static void gc_check_mapping(struct mapping *m) {
	...
	if (1) {
	    switch (m->flags & MAPPING_WEAK) {
	    case MAPPING_WEAK:
		c.kfun = gc_check_weak_svalues;
		c.vfun = gc_check_weak_svalues;
		break;
	    case MAPPING_WEAK_INDICES:
		c.kfun = gc_check_weak_svalues;
		c.vfun = real_gc_check_svalues;
		break;
	    case MAPPING_WEAK_VALUES:
		c.kfun = real_gc_check_svalues;
		c.vfun = gc_check_weak_svalues;
		break;
	    case 0:
		c.kfun = real_gc_check_svalues;
