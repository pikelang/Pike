...
One set of these paired files is included by the selection code
in ../pikecode.[ch]. The selection code is also present in
../program.h (for PIKE_OPCODE_T).

Each of the paired files should implement the following functions/macros:
...
* Used by the compiler:
  ...
  void ins_pointer(INT32 ptr);
	Store a 32bit pointer at the current offset of
	Pike_compiler->new_program->program.

  INT32 read_pointer(INT32 off);
	Read a 32bit pointer from the specified offset of
	Pike_compiler->new_program->program.

  void upd_pointer(INT32 off, INT32 ptr);
	Store a 32bit pointer at the specified offset of
	Pike_compiler->new_program->program.
...
* Used by the compiler:
  ...
  void RELOCATE_program(struct program *p, PIKE_OPCODE_T *new);
	...
	to execute at the new position. The data at 'p'->program
	has already been copied to 'new', so this typically
	just involves applying 'p'->relocations to the data
	at 'new'.
