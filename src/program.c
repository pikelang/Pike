				    ...
				    INT32 flags)
{
  INT32 ret;
  struct svalue tmp;
  SET_SVAL(tmp, T_STRING, 0, string, make_shared_static_string(str, strlen(str), eightbit));
  ret=add_constant(make_shared_static_string(name, strlen(name), eightbit), &tmp, flags);
  free_svalue(&tmp);
  return ret;
}
