...
string(0..255) prf_sha512(string(0..255) secret,
			  ...
			  int len)
{
  return P_hash(Crypto.SHA512, 128, secret, label + seed, len);
}
#endif
	     ...
	     int max_hash_size)
{
  ...
  if (sizeof(algorithms) > 3) {
    ...
    case MODE_gcm:
      if (res->bulk_cipher_algorithm != AES) return 0;
      res->bulk_cipher_algorithm = Crypto.AES.GCM->`();
