...
static mapping(int:array(string)) name_by_exponent = ([
  0 : ({ "", "" }),
  2 : ({ "hundred", "hundred" }),
  3 : ({ "thousand", "thousand" }),
  6 : ({ "million", "million" }),
  ...
  66: ({ "unvigintillion", "undecillion" }),
  69: ({ "duovigintillion", "undecilliard" }),
  72: ({ "trevigintillion", "duodecillion" }),
  75: ({ "quattuorvigintillion", "duodecilliard" }),
  78: ({ "quinvigintillion", "tredecillion" }),
  81: ({ "sexvigintillion", "tredecilliard" }),
  84: ({ "septenvigintillion", "quattuordecillion" }),
  87: ({ "octovigintillion", "quattuordecilliard" }),
  90: ({ "novemvigintillion", "quindecillion" }),
  93: ({ "trigintillion", "quindecilliard" }),
  96: ({ "untrigintillion", "sexdecillion" }),
  99: ({ "duotrigintillion", "sexdecilliard" }),
]);

static mapping(int:string) numbers = ([
  1 : "one",
  2 : "two",
  3 : "three",
  4 : "four",
...
static mapping(int:string) ten_numbers = ([
  2 : "twenty",
  3 : "thirty",
  4 : "fourty",
  5 : "fifty",
...
string int2text(int x, void|int(0..1) lang) {
  array(string|int) internal_int2txt(int rest, int e, int(0..1) lang) {
    int|string parse_low(int n, int(0..1) lang) {
      ...
      if (n<100) {
	int z = n%10;
	return ten_numbers[(n/10)%10]+(z?numbers[z]:"");
      } else return numbers[((n/100)%10)]+name_by_exponent[2][lang] + (parse_low(n%100, lang)||"");
    };

    int mod = rest%1000;
    array(string|int) ret = ({ (mod&&e?name_by_exponent[e][lang]:0), parse_low(mod, lang) });
    rest /= 1000;
    if (!rest) return ret;
    return ret + internal_int2txt(rest, e+3, lang);
  };

  if (zero_type(lang)) lang=1; // default to brittish english
  int(0..1) neg = x < 0;

  array(string|int) res = internal_int2txt((int)abs(x), 0, lang);
  return (neg?"minus ":"") + reverse(res-({0}))*" ";
}
