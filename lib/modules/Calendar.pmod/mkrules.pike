...
void add_abbr(string abbr, string zone)
{
   abbr2zones[abbr] += ({ zone });
}
...
void collect_rules(string file)
{
   ...
   // Current (2017) tzdata files are in UTF-8.
   catch {
     s =  utf8_to_string(s);
   };
...
int main(int ac,array(string) am)
{
   ...
   string orig_names = Stdio.read_bytes("TZnames.pmod");
     ...
     orig_names = utf8_to_string(orig_names);
   };
   ...
   foreach(sort(indices(abbr2zones)), string abbr) {
      t += sprintf("   %q: ({\n", abbr);
      array(string) zones = abbr2zones[abbr];
      foreach(Array.uniq(sort(zones)); int i; string zone)
        t += sprintf("%*s%q,\n", 5, "", zone);
      t += "   }),\n";
   }
