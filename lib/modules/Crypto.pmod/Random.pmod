...
protected class RND {
  ...
  string(0..255) random_string(int len) {
    object key = lock->lock();
    return low_random_string(len);
    key = 0;	// Fix warning.
  }

  string(0..255) low_random_string(int len) {
    String.Buffer buf = String.Buffer(len);
    int new_tick = ticker();
    update( (string(0..255))(new_tick-last_tick), 1, 1 );
    last_tick = new_tick;
    ...
    return (string(0..255))buf;
  }
}

protected string(0..255) rnd_bootstrap(int len) {
  rnd_obj = RND(1);
...
protected function(int:string(0..255)) rnd_func = rnd_bootstrap;

protected string(0..255) rnd_block_bootstrap(int len)
{
  rnd_block_obj = RND(0);
...
protected function(int:string(0..255)) rnd_block_func = rnd_block_bootstrap;
...
//!   truely random.
void add_entropy(string(0..255) data, int entropy) {
  if(rnd_obj)
