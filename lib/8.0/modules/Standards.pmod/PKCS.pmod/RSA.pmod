...
//! hash algorithm. One of @[Crypto.MD2], @[Crypto.MD5], @[Crypto.SHA1],
//! @[Crypto.SHA256], @[Crypto.SHA384] or @[Crypto.SHA512].
Sequence signature_algorithm_id(Crypto.Hash hash)
{
  switch(hash->name())
  {
  case "md2":
    return Sequence( ({ Standards.PKCS.Identifiers.rsa_md2_id, Null() }) );
    break;
  case "md5":
    return Sequence( ({ Standards.PKCS.Identifiers.rsa_md5_id, Null() }) );
    break;
  case "sha1":
    return Sequence( ({ Standards.PKCS.Identifiers.rsa_sha1_id, Null() }) );
    break;
  case "sha256":
    return Sequence( ({ Standards.PKCS.Identifiers.rsa_sha256_id, Null() }) );
    break;
  case "sha384":
    return Sequence( ({ Standards.PKCS.Identifiers.rsa_sha384_id, Null() }) );
    break;
  case "sha512":
    return Sequence( ({ Standards.PKCS.Identifiers.rsa_sha512_id, Null() }) );
    break;
  }
  return 0;
}
